...
rootDataPath = "/Volumes/iot_telemetry/raw/iot-data"
rootAutoloaderConfigPath = "/Volumes/iot_telemetry/raw/iot-data-auto-loader-config-zone"
...
def get_folders(rootDataPath, EXCLUDED_SOURCES):
    folders = [
        item.name.rstrip("/")
        for item in dbutils.fs.ls(rootDataPath)
...
def write_bronze_table(df, checkpointLocation, table_name):
    ...
    query = (
        df.writeStream.option("checkpointLocation", checkpointLocation)
...
def process_folders(folders, rootDataPath):
    successful_folders = []
    failed_folders = []
    ...
    for folder in folders:
        ...
        try:
            checkpointLocation = os.path.join(
                rootAutoloaderConfigPath, "configs", folder, "checkpoint"
            )

            schemaLocation = os.path.join(
                rootAutoloaderConfigPath, "configs", folder, "schema"
            )

            filePath = os.path.join(rootDataPath, folder)
            ...
            df = create_autoloader_stream(schemaLocation, filePath)

            print("Starting Bronze ingestion...")

            query = write_bronze_table(df, checkpointLocation, table_name)
    ...
    if failed_folders:
        ...
        raise RuntimeError(f"Ingestion failed for {len(failed_folders)} source(s).")
